% Script to run random channel + variable frequency stim across arrays
% This script will stimulate active channels at varying amplitudes
% (50, 150, 250, 350 Hz)
%                       Trial Structure
...
stim_paradigm = 'random-channel-frequency-stim';

% Stimulation parameters
num_simul_stim_chans = 1;
all_chans = [1:96 129:160]; % available channels to loop through sequentially
active_chans = [1,2,6,7,9,11,12,33,40,52,53,57,61,73,75,77,79,80,88,146,158,160]';
...
pulse_width=250; % us
stim_freq=[50, 150, 250, 350].'; % Hz
stim_duration=150; % ms
stim_amplitude=[50].'; %uA
...
n_frequencies = length(stim_freq);

total_trials = num_stim_repeats*(catch_trials_per_block + n_stim_elecs*n_frequencies); % total number of planned stim trials
...
stim_chans = repelem(stim_chans, n_frequencies,1);
stim_freq= repmat(stim_freq, n_stim_elecs, 1);
...
for i = 1:num_stim_repeats
    ...
    for channum = 1:length(stim_chans)+catch_trials_per_block
        ...
        chosen_freq = stim_freq(stim_chan_order(channum),:);
        fprintf(stim_record,'%d\t%s\t%d\n',channum + (i-1)*(length(stim_chans)+catch_trials_per_block),num2str(chosen_chan),chosen_freq);
        
        xippmex('stim','enable',0) % disable stim first so step size can be set
        stim_cmd = xippmexStimCmd(chosen_chan,pulse_width,chosen_freq,stim_duration,stim_amplitude,stim_offset);
