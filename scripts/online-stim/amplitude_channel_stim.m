%% Script to run random channel + variable current stim across arrays
...
stim_paradigm = 'random-channel-amplitude-stim';
...
total_trials = num_stim_repeats*(catch_trials_per_block + n_stim_elecs*n_amplitudes); % total number of planned stim trials
...
stim_chans = repelem(stim_chans, n_amplitudes,1);
stim_amplitude = repmat(stim_amplitude, n_stim_elecs, 1);
...
for i = 1:num_stim_repeats
    ...
    for channum = 1:length(stim_chans)+catch_trials_per_block
        ...
        fprintf(stim_record,'%d\t%s\t%d\n',channum + (i-1)*(length(stim_chans)+catch_trials_per_block),num2str(chosen_chan),chosen_amp);
